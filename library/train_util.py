...
def trim_and_resize_if_required(
    random_crop: bool, image: Image.Image, reso, resized_size: Tuple[int, int], upscaler=None, upscale_enable_reso=None
) -> Tuple[np.ndarray, Tuple[int, int], Tuple[int, int, int, int]]:
    image_height, image_width = image.shape[0:2]
    original_size = (image_width, image_height)  # size before resize
    
    if upscaler is not None and image_height * image_width <= upscale_enable_reso:
      image, _ = upscaler.enhance(image, outscale=2)
...
def cache_batch_latents(
    vae: AutoencoderKL, cache_to_disk: bool, image_infos: List[ImageInfo], flip_aug: bool, random_crop: bool, upscaler=None,upscale_enable_reso=None
) -> None:
    r"""
    ...
    for info in image_infos:
        ...
        image, original_size, crop_ltrb = trim_and_resize_if_required(random_crop, image, info.bucket_reso, info.resized_size, upscaler=upscaler,upscale_enable_reso=upscale_enable_reso)
