...
def main(args):
    ...
    if args.model_name_or_path_v2:
        vae2 = model_util.load_vae(args.model_name_or_path_v2, weight_dtype)
        vae2.eval()
        vae2.to(DEVICE, dtype=weight_dtype)
    else:
        vae2 = None
    ...
    for i, image_path in enumerate(tqdm(image_paths, smoothing=0.0)):
        image_key = image_path if args.full_path else os.path.splitext(
        ...
        for j in range(len(buckets_imgs)):
            ...
            if (is_last and len(bucket) > 0) or len(bucket) >= args.batch_size:
                latents = get_latents(
                    vae, [img for _, _, img in bucket], weight_dtype)

                for (image_key, reso, _), latent in zip(bucket, latents):
                    npz_file_name = os.path.splitext(os.path.basename(image_key))[
                        0] if args.full_path else image_key
                    np.savez(os.path.join(
                        args.train_data_dir, npz_file_name), latent)
                if vae2:
                    latents = get_latents(
                        vae2, [img for _, _, img in bucket], weight_dtype)
                    for (image_key, reso, _), latent in zip(bucket, latents):
                        npz_file_name = os.path.splitext(os.path.basename(image_key))[
                            0] if args.full_path else image_key
                        np.savez(os.path.join(args.train_data_dir,
                                 'v2', npz_file_name), latent)

                # flip
                if args.flip_aug:
                    # copyがないとTensor変換できない
                    latents = get_latents(
                        vae, [img[:, ::-1].copy() for _, _, img in bucket], weight_dtype)

                    for (image_key, reso, _), latent in zip(bucket, latents):
                        npz_file_name = os.path.splitext(os.path.basename(image_key))[
                            0] if args.full_path else image_key
                        np.savez(os.path.join(args.train_data_dir,
                                 npz_file_name + '_flip'), latent)
